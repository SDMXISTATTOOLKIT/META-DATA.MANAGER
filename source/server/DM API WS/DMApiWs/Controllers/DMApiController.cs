...
namespace DM_API_WS.Controllers
{
    ...
    public class DMApiController : ControllerBase
    {
        ...
        readonly IOptionsSnapshot<DataProviderNameConfig> _dataProviderNameConfig;

        /// <summary>
        /// Constructor
        /// </summary>
        public DMApiController(IWebHostEnvironment env,
                                IOptionsMonitor<AuthAppOptions> authAppConfig, 
                                IConfiguration configuration, 
                                IUserData userData, 
                                IMemoryCache memoryCache,
                                IOptionsSnapshot<DataProviderNameConfig> dataProviderNameConfig)
        {
            _env = env;
            ...
            _dataProviderNameConfig = dataProviderNameConfig;
        }
        ...
        /// </summary>
        /// <param name="filePath">Path of the file from whom the temp table has been generated.</param>
        [HttpGet("Loader/getDatflowSDMXMLData")]
        public string ImportSDMXMLData(string filePath)
        {
            LoggerUtils.logMethodStarts(_logger, System.Reflection.MethodBase.GetCurrentMethod().Name + $"filePath:{filePath}");

            ILoaderDataProvider loaderData = Factory.LoaderDataProv;

            filePath = normalizeFilePath(filePath);

            var nameresult = loaderData.getDataflowFromXmlImport(filePath);
        ...
        public JsonResult ImportSDMXMLData(string tid, ImportTypeEnum importType, int idCube, [FromBody] DSDParam dsd, string agencyId, string filePath, bool embargo, bool ignoreCuncurrentUpload = false, bool checkFiltAttributes = false)
        {
            ...
            try
            {
                ...
                var watch = System.Diagnostics.Stopwatch.StartNew();
                var sdmxObjects = Utility.SdmxUtils.getSdmxObjectFromXml(dsd.dsd);
                string tempTableName = preloadSDMXML(filePath, sdmxObjects, tid, guidSession);
                watch.Stop();
                long elapsedMs = watch.ElapsedMilliseconds / 1000;
        ...
        public JsonResult GetSDMXMLTablePreview([FromBody] DSDParam dsd, string agencyId, string filePath)
        {
            ...
            var sdmxObjects = Utility.SdmxUtils.getSdmxObjectFromXml(dsd.dsd);
            string tableName = preloadSDMXML(filePath, sdmxObjects, null, guidSession);
        ...
        public JsonResult DDBReset(int cubeId)
        {
            ...
            if (res)
            {
                new AuthManager(_authAppConfig).RemoveDDBDataPermission();
            }
        ...
        public JsonResult GetSeriesForCube(int idCube, [FromBody] DDBDataflow ddbDataflow)
        {
            LoggerUtils.logMethodStarts(_logger, System.Reflection.MethodBase.GetCurrentMethod().Name);

            var res = Factory.UtilityDataProv.GetSeriesForCube(idCube, ddbDataflow);
        ...
        public bool CheckEndPoint()
        {
            ...
            DataProvider.IBuilderDataProvider provider = null;
            try
            {
                provider = Factory.BuilderDataProv;
            }
            catch (Exception)
            {
            ...
            IRMDataProvider rmProvider = null;
            try
            {
                rmProvider = new RMManager.RMDataProvider.RMDataProvider();
            }
            catch (Exception)
            {
            ...
            if (rmProvider != null)
            {
                rmProvider.CheckInizializedRMDB();
            }

            new AuthManager(_authAppConfig).GetAllAgency();
        ...
        /// <param name="guidSession">Guid session from NodeAPI</param>
        /// <returns></returns>
        private string preloadSDMXML(string filePath, ISdmxObjects sdmxObjects, string tid, string guidSession)
        {
            string tempTableName;
            string tidStr = tid ?? "NULL";
            ...
            {
                tempTableName = Factory.LoaderDataProv.preloadSDMXML(filePath, sdmxObjects, tid, _memoryCache, guidSession);
        ...
        public ResultInizializeCheckAuthDb InizializeAuthDb([FromBody] JObject configureData)
        {
            ...
            var ddbXml = _dataProviderNameConfig?.Value?.Default_Data?.CONN_STR;

            var rmXml = _dataProviderNameConfig?.Value?.RM_Data?.CONN_STR;
        ...
        public ResultInizializeCheckAuthDb CheckAuthDB([FromBody] JObject checkData)
        {
            ...
            try
            {
                ddbXml = _configuration["DATA_PROVIDER_NAME:DEFAULT_DATA:CONN_STR"];
            }
            catch
            { }
            ...
            try
            {
                rmXml = _configuration["DATA_PROVIDER_NAME:RM_DATA:CONN_STR"];
            }
            catch
            { }
        ...
        /// </summary>
        /// <returns></returns>
        [HttpGet("Utils/IsAgencyAssignToAnyUser/{agencyCode}")]
        public bool IsAgencyAssignToAnyUser(string agencyCode)
        {
            LoggerUtils.logMethodStarts(_logger, System.Reflection.MethodBase.GetCurrentMethod().Name);
            var result = new AuthManager(_authAppConfig).IsAgencyAssignToAnyUser(agencyCode);
            LoggerUtils.logMethodEndsSuccess(_logger, System.Reflection.MethodBase.GetCurrentMethod().Name);
            return result;
        }
        ...
        {
            return Path.Combine(_configuration["DMApiSettings:IMPORT_FILE_BASE_DIR"],
                                        filePath.TrimStart('\\').TrimStart('/').Replace(":", ""));
        }
    }
}
