...
namespace DM_API_WS.Controllers
{
    ...
    public class RMController : ControllerBase
    {
        ...
        public ReportType UpdateReport([FromBody] RMParam rmParams, int metadataFlowId, string msd=null)
        {
            ...
            ReportType rUpdated = rmProvider.UpdateReport(metadataFlowId, metadataSet.Report[0], xDomMSD);

            try
            {
                string CacheDir = _cacheConfig.Value.Dir;
                RMCacheManager _rmCacheManager = new RMCacheManager(CacheDir);
                _rmCacheManager.deleteRMMetadata(metadataSet.setID, metadataSet.Report[0].id);
            }
            catch (Exception e)
            {
                STLoggerFactory.Logger.Log("Error to remove data from cache", e, LogLevelEnum.Warn);
            }
        ...
        public MetadataSetType UpdateMetadataSet([FromBody] RMParam rmParams)
        {
            ...
            MetadataSetType mdsUpdated = rmProvider.UpdateMetadataSet(metadataSet, xDomMSD);

            try {
                string CacheDir = _cacheConfig.Value.Dir;
                RMCacheManager _rmCacheManager = new RMCacheManager(CacheDir);
                _rmCacheManager.deleteRMMetadata(metadataSet.setID, null);
            }
            catch (Exception e)
            {
                STLoggerFactory.Logger.Log("Error to remove data from cache", e, LogLevelEnum.Warn);
